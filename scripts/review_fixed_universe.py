...
from app.universe import build_fixed_universe, review_fixed_universe


def main() -> int:
    parser = argparse.ArgumentParser(description="Review the fixed universe without changing configuration.")
    ...
    as_of = datetime.fromisoformat(f"{trade_date}T16:00:00").replace(tzinfo=timezone)
    try:
        report = review_fixed_universe(universe, source, as_of).to_dict()
